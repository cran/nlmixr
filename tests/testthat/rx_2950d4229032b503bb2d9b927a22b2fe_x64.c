...
extern void rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr();



// prj-specific differential eqns
void rx_2950d4229032b503bb2d9b927a22b2fe_x64_dydt(unsigned int _neq, double t, double *__zzStateVar__, double *__DDtStateVar__)
{
double 
		centr,
	_ETA_1_,
	...
	_THETA_3_ = _par_ptr(4);

	centr = __zzStateVar__[0];

	__DDtStateVar__[0]=_InfusionRate(0)+0.0+-centr*exp(_ETA_1_+_THETA_1_)*exp(-_ETA_2_-_THETA_2_);
	rx_pred_=centr*exp(-_ETA_2_-_THETA_2_);
	rx_r_=Rx_pow_di(_THETA_3_,2)*Rx_pow_di(centr,2)*exp(-2*_ETA_2_-2*_THETA_2_);
    _dadt_counter_inc();
}

// Jacobian derived vars
void rx_2950d4229032b503bb2d9b927a22b2fe_x64_calc_jac(unsigned int _neq, double t, double *__zzStateVar__, double *__PDStateVar__, unsigned int __NROWPD__) {
  _jac_counter_inc();
}
// Functional based initial conditions.
void rx_2950d4229032b503bb2d9b927a22b2fe_x64_inis(SEXP _ini_sexp){
...
double 
	...
	rx_pred_=centr*exp(-_ETA_2_-_THETA_2_);
	rx_r_=Rx_pow_di(_THETA_3_,2)*Rx_pow_di(centr,2)*exp(-2*_ETA_2_-2*_THETA_2_);
  __zzStateVar__[0]=centr;
}
// prj-specific derived vars
void rx_2950d4229032b503bb2d9b927a22b2fe_x64_calc_lhs(double t, double *__zzStateVar__, double *_lhs) {
double 
		__DDtStateVar_0__,
	centr,
	_ETA_1_,
	...
	__DDtStateVar_0__=_InfusionRate(0)+0.0+-centr*exp(_ETA_1_+_THETA_1_)*exp(-_ETA_2_-_THETA_2_);
	rx_pred_=centr*exp(-_ETA_2_-_THETA_2_);
	rx_r_=Rx_pow_di(_THETA_3_,2)*Rx_pow_di(centr,2)*exp(-2*_ETA_2_-2*_THETA_2_);
...
extern SEXP rx_2950d4229032b503bb2d9b927a22b2fe_x64_model_vars(){
	SEXP lst      = PROTECT(allocVector(VECSXP, 12));
	SEXP names    = PROTECT(allocVector(STRSXP, 12));
	SEXP params   = PROTECT(allocVector(STRSXP, 5));
	SEXP lhs      = PROTECT(allocVector(STRSXP, 2));
	SEXP state    = PROTECT(allocVector(STRSXP, 1));
	SEXP stateRmS = PROTECT(allocVector(INTSXP, 1));
	...
	SET_STRING_ELT(params,4,mkChar("THETA[3]"));
	SET_STRING_ELT(state,0,mkChar("centr"));
	stateRm[0] = 0;
	SET_STRING_ELT(modeln,0,mkChar("model"));
	SET_STRING_ELT(model,0,mkChar("d/dt(centr)=rxRate(centr)+-centr*exp(ETA[1]+THETA[1])*exp(-ETA[2]-THETA[2]);\nrx_pred_=centr*exp(-ETA[2]-THETA[2]);\nrx_r_=Rx_pow_di(THETA[3],2)*Rx_pow_di(centr,2)*exp(-2*ETA[2]-2*THETA[2]);\n"));
	SET_STRING_ELT(modeln,1,mkChar("normModel"));
	SET_STRING_ELT(model,1,mkChar("d/dt(centr)=rxRate(centr)+-centr*exp(ETA[1]+THETA[1])*exp(-ETA[2]-THETA[2]);\nrx_pred_=centr*exp(-ETA[2]-THETA[2]);\nrx_r_=Rx_pow_di(THETA[3],2)*Rx_pow_di(centr,2)*exp(-2*ETA[2]-2*THETA[2]);\n"));
	SET_STRING_ELT(modeln,2,mkChar("parseModel"));
	SET_STRING_ELT(model,2,mkChar("__DDtStateVar__[0] = _InfusionRate(0) + 0.0+-centr*exp(_ETA_1_+_THETA_1_)*exp(-_ETA_2_-_THETA_2_);\nrx_pred_=centr*exp(-_ETA_2_-_THETA_2_);\nrx_r_=Rx_pow_di(_THETA_3_,2)*Rx_pow_di(centr,2)*exp(-2*_ETA_2_-2*_THETA_2_);\n"));
	SET_STRING_ELT(modeln,3,mkChar("expandModel"));
	SET_STRING_ELT(model,3,mkChar("d/dt(centr)=rxRate(centr)+-centr*exp(ETA[1]+THETA[1])*exp(-ETA[2]-THETA[2]);\nrx_pred_=centr*exp(-ETA[2]-THETA[2]);\nrx_r_=Rx_pow_di(THETA[3],2)*Rx_pow_di(centr,2)*exp(-2*ETA[2]-2*THETA[2]);\n"));
	...
	SET_STRING_ELT(mmd5,0,mkChar("f2104ead2577b75c5961fbfe0246a2f6"));
	SET_STRING_ELT(mmd5n,1,mkChar("parsed_md5"));
	SET_STRING_ELT(mmd5,1,mkChar("2950d4229032b503bb2d9b927a22b2fe"));
	SET_STRING_ELT(trann,0,mkChar("jac"));
	SET_STRING_ELT(tran,0,mkChar("fullint"));
	SET_STRING_ELT(trann,1,mkChar("prefix"));
	SET_STRING_ELT(tran, 1,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_"));
	SET_STRING_ELT(trann,2,mkChar("dydt"));
	SET_STRING_ELT(tran, 2,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_dydt"));
	SET_STRING_ELT(trann,3,mkChar("calc_jac"));
	SET_STRING_ELT(tran, 3,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_calc_jac"));
	SET_STRING_ELT(trann,4,mkChar("calc_lhs"));
	SET_STRING_ELT(tran, 4,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_calc_lhs"));
	SET_STRING_ELT(trann,5,mkChar("model_vars"));
	SET_STRING_ELT(tran, 5,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_model_vars"));
	SET_STRING_ELT(trann,6,mkChar("ode_solver"));
	SET_STRING_ELT(tran, 6,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver"));
	SET_STRING_ELT(trann,7,mkChar("ode_solver_sexp"));
	SET_STRING_ELT(tran, 7,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_sexp"));
	SET_STRING_ELT(trann,8,mkChar("ode_solver_0_6"));
	SET_STRING_ELT(tran, 8,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6"));
	SET_STRING_ELT(trann,9,mkChar("ode_solver_focei_eta"));
	SET_STRING_ELT(tran, 9,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_focei_eta"));
	SET_STRING_ELT(trann,10,mkChar("ode_solver_ptr"));
	SET_STRING_ELT(tran, 10,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr"));
	SET_STRING_ELT(trann,11,mkChar("inis"));
	SET_STRING_ELT(tran, 11,mkChar("rx_2950d4229032b503bb2d9b927a22b2fe_x64_inis"));
...
extern void rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver(
                    int *neq,
                    double *theta,      //order:
                    double *time,
                    int *evid,
  ...
  rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr();
  _old_c(neq, theta, time, evid, ntime, inits, dose, ret, atol, rtol, stiff, transit_abs, nlhs, lhs, rc);
}

void rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6(int *neq,
                        double *theta,  //order:
                        double *time,
                        int *evid,
                        int *ntime,
...
extern void rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr  (){
  _assign_fn_pointers(rx_2950d4229032b503bb2d9b927a22b2fe_x64_dydt , rx_2950d4229032b503bb2d9b927a22b2fe_x64_calc_lhs , rx_2950d4229032b503bb2d9b927a22b2fe_x64_calc_jac, rx_2950d4229032b503bb2d9b927a22b2fe_x64_inis, 2 , 22,
#ifdef __DEBUG__
                      1
#else
                      0
...
extern SEXP rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_sexp (// Parameters
                                 SEXP sexp_theta,
                                 SEXP sexp_inits,
                                 SEXP sexp_lhs,
				 // Events
...
static R_NativePrimitiveArgType rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solverrx_t[] = {
...
static R_NativePrimitiveArgType  rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6rx_t[] = {
  //*neq, *theta, *time,  *evid, *ntime, *inits,   *dose,   *ret,     *atol,  *rtol,   *stiff, *transit_abs, *nlhs, *lhs, *rc
...
void R_init_rx_2950d4229032b503bb2d9b927a22b2fe_x64 (DllInfo *info){
  ...
  R_RegisterCCallable("rx_2950d4229032b503bb2d9b927a22b2fe_x64","rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver",       (DL_FUNC) rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver);
  R_RegisterCCallable("rx_2950d4229032b503bb2d9b927a22b2fe_x64","rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_sexp",  (DL_FUNC) rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_sexp);
  R_RegisterCCallable("rx_2950d4229032b503bb2d9b927a22b2fe_x64","rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6",   (DL_FUNC) rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6);
  R_RegisterCCallable("rx_2950d4229032b503bb2d9b927a22b2fe_x64","rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr",   (DL_FUNC) rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr);

  /* R_registerRoutines(info, NULL, NULL, NULL, NULL); */
  /* R_useDynamicSymbols(info,TRUE); */

  static const R_CMethodDef cMethods[] = {
    {"rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver", (DL_FUNC) &rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver, 15, rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solverrx_t},
    {"rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6", (DL_FUNC) &rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6, 21, rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_0_6rx_t},
    {NULL, NULL, 0, NULL}
  };
  
  R_CallMethodDef callMethods[]  = {
    {"rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr", (DL_FUNC) &rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_ptr, 0},
    {"rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_sexp", (DL_FUNC) &rx_2950d4229032b503bb2d9b927a22b2fe_x64_ode_solver_sexp, 23},
    {"rx_2950d4229032b503bb2d9b927a22b2fe_x64_model_vars", (DL_FUNC) &rx_2950d4229032b503bb2d9b927a22b2fe_x64_model_vars, 0},
    {"rx_2950d4229032b503bb2d9b927a22b2fe_x64_inis", (DL_FUNC) &rx_2950d4229032b503bb2d9b927a22b2fe_x64_inis, 1},
    {NULL, NULL, 0}
  };
